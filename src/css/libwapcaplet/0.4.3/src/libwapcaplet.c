...
static lwc_error lwc__intern(const char *s, size_t slen,lwc_string **ret,lwc_hasher hasher,lwc_strncmp compare,lwc_memcpy copy)
{
	lwc_hash h;
	lwc_hash bucket;
	lwc_string *str;
...
lwc_string_destroy(lwc_string *str)
{
	...
	*(str->prevptr) = str->next;
	...
	if (str->insensitive != NULL && str->refcnt == 0) lwc_string_unref(str->insensitive);
