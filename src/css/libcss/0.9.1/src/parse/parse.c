...
#include "F:\Netsurf-demo\libcss\0.9.1\src\utils\utils.h"
#undef DEBUG_STACK
#undef DEBUG_EVENTS
...
css_error css__parser_setopt(css_parser *parser, css_parser_opttype type,
  ...
  switch (type) {
	case CSS_PARSER_QUIRKS: parser->quirks = params->quirks;
	  break;
	case CSS_PARSER_EVENT_HANDLER: parser->event = params->event_handler.handler;
...
css_error css__parser_parse_chunk(css_parser *parser, const uint8_t *data,
  ...
  perror = parserutils_inputstream_append(parser->stream, data, len);//获得数据流
  if (perror != PARSERUTILS_OK)
	return css_error_from_parserutils_error(perror);
  do {
	state = parserutils_stack_get_current(parser->states);//得到当前状态
	if (state == NULL)
	  break;
	error = parseFuncs[state->state](parser);//通过当前状态和函数指针，调用函数
  } while (error == CSS_OK);
...
css_error css__parser_create_internal(const char *charset,
  ...
  perror = parserutils_inputstream_create(charset, cs_source,
										  css__charset_extract, &p->stream);//输入流
  if (perror != PARSERUTILS_OK) {
	free(p);
	return css_error_from_parserutils_error(perror);
  }

  error = css__lexer_create(p->stream, &p->lexer);//创建lex
  if (error != CSS_OK) {
  ...
  perror = parserutils_stack_create(sizeof(char),
									STACK_CHUNK, &p->open_items);//创建成员栈
  if (perror != PARSERUTILS_OK) {
  ...
  perror = parserutils_stack_push(p->states, (void *)&initial);//初始化栈
  if (perror != PARSERUTILS_OK) {
...
css_error getToken(css_parser *parser, const css_token **token) {
  ...
  if (parser->pushback != NULL) {
	*token = parser->pushback;
	...
	error = css__lexer_get_token(parser->lexer, &t);//调用词法分析器，得到下一个token
	if (error != CSS_OK)
	  return error;
...
css_error parseStart(css_parser *parser) {
  ...
  parser_state *state = parserutils_stack_get_current(parser->states);//得到当前状态
  css_error error = CSS_OK;

  /* start -> ws stylesheet EOF */

  switch (state->substate) {//判断当前状态的子状态
	case Initial://初始化
#if !defined(NDEBUG) && defined(DEBUG_EVENTS)
	  printf("Begin stylesheet\n");
#endif
	  if (parser->event != NULL) {//如果有其他插件，先执行
		parser->event(CSS_PARSER_START_STYLESHEET, NULL, parser->event_pw);
	  }

	  error = eatWS(parser);//吃掉后面的空格
	  if (error != CSS_OK)
		return error;
	  state->substate = AfterWS;//改状态为消除空格后
	  /* Fall through */
	case AfterWS: {
	...
	case AfterStylesheet: error = expect(parser, CSS_TOKEN_EOF);
	  if (error != CSS_OK)
		return error;
  ...
  discard_tokens(parser);//丢掉现有的token

  return done(parser);//弹出当前状态值
}
...
css_error parseStylesheet(css_parser *parser) {
  ...
  while (1) {
	switch (state->substate) {
	  case Initial: error = getToken(parser, &token);//得到一个token
		if (error != CSS_OK)
		  return error;

		switch (token->type) {
		  case CSS_TOKEN_EOF: error = pushBack(parser, token);
			if (error != CSS_OK)
			  return error;
			...
			return done(parser);
		  case CSS_TOKEN_CDO:
		  case CSS_TOKEN_CDC: break;
		  default: {
			parser_state to = {sStatement, Initial};
			parser_state subsequent = {sStylesheet, WS};//跟之前一样的操作，转换状态

			error = pushBack(parser, token);//插入token
			if (error != CSS_OK)
			  return error;
...
css_error parseStatement(css_parser *parser) {
  ...
  error = getToken(parser, &token);//得到token,由于上面pushback有一个token,直接使用该token。
  if (error != CSS_OK)
	return error;
  ...
  error = pushBack(parser, token);//插入token,意味着又将原token插入了。
  if (error != CSS_OK)
	return error;
...
css_error parseRuleset(css_parser *parser) {
  ...
  switch (state->substate) {
	case Initial: discard_tokens(parser);
...
css_error parseRulesetEnd(css_parser *parser) {
  ...
  switch (state->substate) {
	case Initial: error = getToken(parser, &token);
	  if (error != CSS_OK)
		return error;
	  ...
	  if (token->type != CSS_TOKEN_CHAR ||
		  lwc_string_length(token->idata) != 1 ||
		  (lwc_string_data(token->idata)[0] != '}' &&
			  lwc_string_data(token->idata)[0] != ';')) {//如果不是;或}，说明不是decl-list
	...
	case Brace: error = getToken(parser, &token);
	  if (error != CSS_OK)
		return error;
