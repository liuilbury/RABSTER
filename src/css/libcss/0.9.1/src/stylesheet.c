...
css_error css_stylesheet_create(const css_stylesheet_params *params,
  ...
  sheet = calloc(1, sizeof(css_stylesheet)); //申请内存
  if (sheet == NULL)
	return CSS_NOMEM;

  error = css__propstrings_get(&sheet->propstrings);  //得到成员的值
  if (error != CSS_OK) {
	free(sheet);
	return error;
  }

  sheet->inline_style = params->inline_style;    //是否是内联的
  ...
  sheet->quirks_allowed = params->allow_quirks;//是否运行怪癖
  ...
  error = css__language_create(sheet, sheet->parser,&sheet->parser_frontend);//创建语法生成器
  if (error != CSS_OK) {
	css__parser_destroy(sheet->parser);
	css__propstrings_unref();
	free(sheet);
...
css_error css__stylesheet_selector_destroy(css_stylesheet *sheet,
  ...
  for (c = selector->combinator; c != NULL; c = d) {
	...
	for (detail = &c->data; detail;) {
	  if (detail->qname.ns != NULL) lwc_string_unref(detail->qname.ns);
...
css_error css__stylesheet_selector_append_specific(css_stylesheet *sheet,
  ...
  *(d = &(&temp->data)[num_details + 1]) = *detail;
  ...
  switch (detail->type) {
	...
	case CSS_SELECTOR_ATTRIBUTE_SUBSTRING: (*parent)->specificity += CSS_SPECIFICITY_C;
	  break;
	case CSS_SELECTOR_ID: (*parent)->specificity += CSS_SPECIFICITY_B;
	  break;
	case CSS_SELECTOR_PSEUDO_ELEMENT:
	case CSS_SELECTOR_ELEMENT: (*parent)->specificity += CSS_SPECIFICITY_D;
	  break;
  }
...
css_error css__stylesheet_rule_create(css_stylesheet *sheet, css_rule_type type,
  ...
  switch (type) {
	case CSS_RULE_UNKNOWN: required = sizeof(css_rule);
	  break;
	case CSS_RULE_SELECTOR: required = sizeof(css_rule_selector);
	  break;
	case CSS_RULE_CHARSET: required = sizeof(css_rule_charset);
	  break;
	case CSS_RULE_IMPORT: required = sizeof(css_rule_import);
	  break;
	case CSS_RULE_MEDIA: required = sizeof(css_rule_media);
	  break;
	case CSS_RULE_FONT_FACE: required = sizeof(css_rule_font_face);
	  break;
	case CSS_RULE_PAGE: required = sizeof(css_rule_page);
	  break;
  }
...
css_error _add_selectors(css_stylesheet *sheet, css_rule *rule) {
  ...
  switch (rule->type) {
	case CSS_RULE_SELECTOR: {
	  ...
	  for (i = 0; i < rule->items; i++) {//遍历rule的选择器，hash，结果储存在sheet->selectors
		css_selector *sel = s->selectors[i];
