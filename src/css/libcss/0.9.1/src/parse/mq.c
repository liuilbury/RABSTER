	...
	css_mq_cond_or_feature *cond_or_feature)
{
	if (cond_or_feature != NULL)
	{
		...
		{
			case CSS_MQ_FEATURE: css__mq_feature_destroy(cond_or_feature->data.feat);
				break;
			case CSS_MQ_COND: css__mq_cond_destroy(cond_or_feature->data.cond);
				break;
		}
											...
											bool until, const char until_char)
{
	...
	while (true)
	{
		...
		switch (token->type)
		{
			case CSS_TOKEN_INVALID_STRING: return CSS_INVALID;
								...
								css_mq_query **media_out)
{
	css_error err;
	css_parser *parser;
	css_mq_parse_ctx ctx = {.strings = strings,};
	...
	err = css__parser_parse_chunk(parser, (const uint8_t *)"@media ", strlen("@media "));//解析@media,调用事件
	...
	err = css__parser_parse_chunk(parser, mq, len);//解析是哪些media
	if (err != CSS_OK && err != CSS_NEEDDATA)
	{
		css__parser_destroy(parser);
		return err;
