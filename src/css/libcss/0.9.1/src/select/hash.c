	...
	const css_selector* selector)//
{
	...
	//遍历selector，按照id->class->element->*的优先级来进行插入到对应的hash桶内
	///可优化。
		...
		while (head != NULL)
		{
			...
			if (match && RULE_HAS_BYTECODE(head))
			{
				if (css_bloom_in_bloom(head->sel_chain_bloom, req->node_bloom)
					&& mq_rule_good_for_media(head->sel->rule, req->media))
				{
					/* Found a match */
					break;
				}
	...
	if (head->sel != NULL)
	{
		...
		while (head != NULL)
		{
			...
			if (n != NULL)
			{
				...
				if (match && RULE_HAS_BYTECODE(head))
				{
					if (css_bloom_in_bloom(head->sel_chain_bloom, req->node_bloom) &&//bloom
						_chain_good_for_element_name(head->sel, &(req->qname), req->uni) &&
						mq_rule_good_for_media(head->sel->rule, req->media))//media一样
					{
						/* Found a match */
						break;
					}
...
lwc_string* _class_name(const css_selector* selector)
{
	...
	const css_selector* ss=selector;
	while(ss->data.comb!= CSS_COMBINATOR_NONE)
	{
		//printf("%s\n", lwc_string_data(ss->combinator->data.qname.name));
		ss=ss->combinator;
	}
	do
	{
	...
	const css_selector* selector)
{
	...
	else
	{
		...
		do
		{
			//按选择器的优先级进行升序排序,例如.div->.div.class->.div.class.class;
