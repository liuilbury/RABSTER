...
css_error parseStylesheet(css_parser* parser)
{
	...
	while (1)
	{
		...
		{
		case Initial:
			error = getToken(parser, &token);//得到一个token
			if (error != CSS_OK)
				return error;
...
css_error parseRulesetEnd(css_parser* parser)
{
	enum
	{
		Initial = 0, DeclList = 1, Brace = 2, WS = 3
	};
	parser_state* state = parserutils_stack_get_current(parser->states);
	const css_token* token;
	css_error error;
...
css_error parseAtRule(css_parser* parser)
{
	enum
	{
		Initial = 0, WS = 1, Any = 2, AfterAny = 3
	};
	parser_state* state = parserutils_stack_get_current(parser->states);
...
css_error parseBlock(css_parser* parser)
{
	enum
	{
		Initial = 0, WS = 1, Content = 2, Brace = 3, WS2 = 4
	};
	parser_state* state = parserutils_stack_get_current(parser->states);
	const css_token* token;
	css_error error;
...
css_error parseDeclaration(css_parser* parser)
{
	enum
	{
		Initial = 0, Colon = 1, WS = 2, AfterValue1 = 3
	};
	parser_state* state = parserutils_stack_get_current(parser->states);
	const css_token* token;
	css_error error;
...
css_error parseAny1(css_parser* parser)
{
	enum
	{
		Initial = 0, AfterAny = 1, AfterAny0 = 2
	};
	parser_state* state = parserutils_stack_get_current(parser->states);
	const css_token* token;
	css_error error;
...
css_error parseAny(css_parser* parser)
{
	enum
	{
		Initial = 0, WS = 1, AfterAny0 = 2, WS2 = 3
	};
	parser_state* state = parserutils_stack_get_current(parser->states);
	const css_token* token;
	css_error error;
