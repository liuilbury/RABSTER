		...
		const parserutils_vector *vector)
{
	...
	perror = parserutils_stack_push(c->context, (void *) &entry);//将状态推入context堆栈
	if (perror != PARSERUTILS_OK) {
		return css_error_from_parserutils_error(perror);
	}
...
css_error handleStartRuleset(css_language *c, const parserutils_vector *vector)
{
	...
	cur = parserutils_stack_get_current(c->context);//查询有没有父节点
	if (cur != NULL && cur->type != CSS_PARSER_START_STYLESHEET)
		parent_rule = cur->data;//如果有，继承

	error = css__stylesheet_rule_create(c->sheet, CSS_RULE_SELECTOR, &rule);//创建一个新的rule
	if (error != CSS_OK)
		return error;

	if (vector != NULL) {
		/* Parse selectors, if there are any */
		error = parseSelectorList(c, vector, rule);//解析选择器列表
		if (error != CSS_OK) {
	...
	perror = parserutils_stack_push(c->context, (void *) &entry);//状态推入stack
	if (perror != PARSERUTILS_OK) {
		css__stylesheet_rule_destroy(c->sheet, rule);
		return css_error_from_parserutils_error(perror);
	}

	error = css__stylesheet_add_rule(c->sheet, rule, parent_rule);//添加rule
	if (error != CSS_OK) {
		...
		const parserutils_vector *vector, int *ctx, css_rule *rule)
{
	...
	for (i = FIRST_PROP; i <= LAST_PROP; i++) {//遍历属性表，找到对应的属性
		bool match = false;
