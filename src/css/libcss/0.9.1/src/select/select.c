									   ...
									   void *clone_node, void *libcss_node_data)
{
	...
	switch (action)
	{
		case CSS_NODE_DELETED: css__destroy_node_data(node_data);
			break;
									  ...
									  css_origin origin, const char *media)
{
	...
	error = css_parse_media_query(sheet->propstrings,
								  (const uint8_t *)media,
								  (media == NULL) ? 0 : strlen(media), &mq);//处理当前平台media
	if (error == CSS_NOMEM)
	{
		return error;
	}
						   ...
						   css_select_results **result)
{
	...
	error = handler->parent_node(pw, node, &parent);//是否有爹
	if (error != CSS_OK)
		return error;

	error = css_select__initialise_selection_state(&state, node, parent, media, handler, pw);//初始化
	if (error != CSS_OK)
		return error;

	/* Fetch presentational hints */
	error = handler->node_presentational_hint(pw, node, &nhints, &hints);//获取提示？
	if (error != CSS_OK)
		goto cleanup;
	if (nhints > 0)
	{
	...
	error = css_select_style__get_sharable_node_data(node, &state, &share);//检查是否可以共享之前的节点
	if (error != CSS_OK)
	{
		goto cleanup;
	}
	 ...
	 */
	error = css__computed_style_create(&state.results->styles[CSS_PSEUDO_ELEMENT_NONE]);//创建一个计算样式
	if (error != CSS_OK)
	{
		goto cleanup;
	}
	 ...
	 * are not disabled */
	for (i = 0; i < ctx->n_sheets; i++)//遍历样式表
	{
		const css_select_sheet s = ctx->sheets[i];
		if (mq__list_match(s.media, media) &&
			s.sheet->disabled == false)//如果这个样式表符合media而且没有被禁用，则进行查询
		{
...
{
	if (ctx->universal != NULL) lwc_string_unref(ctx->universal);
	if (ctx->first_child != NULL) lwc_string_unref(ctx->first_child);
	if (ctx->link != NULL) lwc_string_unref(ctx->link);
	if (ctx->visited != NULL) lwc_string_unref(ctx->visited);
	if (ctx->hover != NULL) lwc_string_unref(ctx->hover);
	if (ctx->active != NULL) lwc_string_unref(ctx->active);
	if (ctx->focus != NULL) lwc_string_unref(ctx->focus);
	if (ctx->nth_child != NULL) lwc_string_unref(ctx->nth_child);
	if (ctx->nth_last_child != NULL) lwc_string_unref(ctx->nth_last_child);
	if (ctx->nth_of_type != NULL) lwc_string_unref(ctx->nth_of_type);
	if (ctx->nth_last_of_type != NULL) lwc_string_unref(ctx->nth_last_of_type);
	if (ctx->last_child != NULL) lwc_string_unref(ctx->last_child);
	if (ctx->first_of_type != NULL) lwc_string_unref(ctx->first_of_type);
	if (ctx->last_of_type != NULL) lwc_string_unref(ctx->last_of_type);
	if (ctx->only_child != NULL) lwc_string_unref(ctx->only_child);
	if (ctx->only_of_type != NULL) lwc_string_unref(ctx->only_of_type);
	if (ctx->root != NULL) lwc_string_unref(ctx->root);
	if (ctx->empty != NULL) lwc_string_unref(ctx->empty);
	if (ctx->target != NULL) lwc_string_unref(ctx->target);
	if (ctx->lang != NULL) lwc_string_unref(ctx->lang);
	if (ctx->enabled != NULL) lwc_string_unref(ctx->enabled);
	if (ctx->disabled != NULL) lwc_string_unref(ctx->disabled);
	if (ctx->checked != NULL) lwc_string_unref(ctx->checked);
	if (ctx->first_line != NULL) lwc_string_unref(ctx->first_line);
	if (ctx->first_letter != NULL) lwc_string_unref(ctx->first_letter);
	if (ctx->before != NULL) lwc_string_unref(ctx->before);
	if (ctx->after != NULL) lwc_string_unref(ctx->after);
}
							...
							css_origin origin, css_select_state *state)
{
	...
	do//遍历rule集合
	{
		...
		if (rule == s->rule_list)//找到第一个非字符rule
		{
			while (rule != NULL && rule->type == CSS_RULE_CHARSET)
				rule = rule->next;
		}

		if (rule != NULL && rule->type == CSS_RULE_IMPORT)//如果是improt，导入
		{
		...
		else
		{
			...
			/* Process this sheet */
			state->sheet = s;//处理完import后
			state->current_origin = origin;

			error = match_selectors_in_sheet(ctx, s, state);//查询
			if (error != CSS_OK)
				return error;
	...
	css_select_font_faces_state *state)
{
	if (mq_rule_good_for_media((const css_rule *)rule, state->media))
	{
			...
			correct_family)
		{
			...
			switch (origin)
			{
				case CSS_ORIGIN_UA: faces = &state->ua_font_faces;
					break;
				case CSS_ORIGIN_USER: faces = &state->user_font_faces;
					break;
				case CSS_ORIGIN_AUTHOR: faces = &state->author_font_faces;
					break;
			}
								   ...
								   const css_stylesheet *sheet, css_select_state *state)
{
	...
	error = css__selector_hash_find(sheet->selectors, &req, &node_iterator, &node_selectors);//找到名称匹配的选择器链
	if (error != CSS_OK)
		goto cleanup;
	//匹配class选择器
	...
	while (_selectors_pending(node_selectors, id_selectors, class_selectors, n_classes, univ_selectors))
	{
		...
		switch (src.source)
		{
			...
			case CSS_SELECT_RULE_SRC_CLASS: req.class = state->classes[src.class];
								 ...
								 void *node, void **next_node)
{
	...
	do
	{
		...
		switch (type)
		{
			...
			case CSS_COMBINATOR_NONE: break;
		}
									 ...
									 void **next_node)
{
	...
	*rejected_by_cache = false;
	...
	do
	{
		...
		{
			case CSS_COMBINATOR_ANCESTOR:
			case CSS_COMBINATOR_PARENT: error = state->handler->parent_node(state->pw, n, &n);
				if (error != CSS_OK)
					return error;
				break;
			case CSS_COMBINATOR_SIBLING:
			case CSS_COMBINATOR_GENERIC_SIBLING: error = state->handler->sibling_node(state->pw, n, &n);
				if (error != CSS_OK)
					return error;
	...
	switch (detail->type)
	{
		...
		case CSS_SELECTOR_ID:
			error = state->handler->node_has_id(state->pw, node,
												detail->qname.name, match);
			break;
		case CSS_SELECTOR_PSEUDO_CLASS: error = state->handler->node_is_root(state->pw, node, &is_root);
			if (error != CSS_OK)
				return error;
			...
			break;
		case CSS_SELECTOR_PSEUDO_ELEMENT: *match = true;
...
css_error cascade_style(const css_style *style, css_select_state *state)
{
	...
	while (s.used > 0)
	{
		...
		error = prop_dispatch[op].cascade(opv, &s, state);//修改属性
		if (error != CSS_OK)
			return error;
	}
